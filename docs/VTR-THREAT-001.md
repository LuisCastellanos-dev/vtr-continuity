...
| S-3 | `rpi/proxy.py` | Suplantación del cliente que envía `POST /events` — el endpoint no requiere autenticación de ningún tipo | 🔴 Sin mitigación | `receive_events()` (línea 198) no tiene ningún parámetro, header, ni dependency de FastAPI que verifique quién hace la solicitud — cualquier proceso con acceso de red al puerto del proxy puede encolar eventos como si fuera un HMI legítimo. **Nota de precisión:** `VTR_API_KEY` (línea 49, 66-69) no protege este endpoint — esa clave es la que el proxy usa para autenticarse *ante el servidor central* al sincronizar hacia afuera (`SyncConfig(api_key=...)`, línea 76), un canal completamente distinto. Si `VTR_API_KEY` está vacío, el único efecto documentado es que "el proxy correrá pero no podrá sincronizar" — la ingesta de eventos entrantes nunca estuvo protegida por esta variable, con o sin ella configurada. |
...
| T-3 | `rpi/proxy.py` (`DELETE /queue`) | Borrado/alteración no autorizada de la cola de eventos pendientes de sincronizar | 🟡 Parcial | El endpoint está protegido solo por la variable de entorno `VTR_DEBUG=true`, no por autenticación — la línea 285 del código deja explícito que "solo disponible con VTR_DEBUG=true", lo cual es un interruptor de configuración, no un control de acceso. Si `VTR_DEBUG` queda activo por error en un despliegue real, cualquier cliente con acceso de red al proxy puede vaciar la cola sin autenticarse. |
...
| R-3 | `rpi/proxy.py` | Ningún evento entrante queda atribuido a un emisor verificado | 🔴 Sin mitigación | Mismo hallazgo que S-3: `receive_events()` no autentica al emisor. Sin esa garantía, no hay forma de afirmar técnicamente que el "origen" registrado en un evento corresponde al emisor real — cualquier repudio de origen es imposible de refutar. |
...
| I-3 | `rpi/proxy.py` (`GET /stats`, `GET /health`) | Exposición de métricas operativas sin autenticación | 🔴 Sin mitigación | Igual que `receive_events()`, ninguno de estos dos endpoints GET tiene mecanismo de autenticación en su definición — son accesibles a cualquiera con acceso de red al proxy, revelando estado de cola, sincronización, y salud del sistema a un observador no autenticado. Información útil para planear un ataque de timing o DoS. |
...
| D-3 | `rpi/proxy.py` | Inundación de `POST /events` sin rate limiting visible en el código | 🔴 Sin mitigación | No se encontró ningún mecanismo de rate limiting, throttling, ni circuit breaker en `proxy.py` — combinado con S-3 (sin autenticación obligatoria), un atacante con acceso de red puede enviar volumen arbitrario de eventos, presionando tanto la cola SQLite local como, en cascada, el `SyncManager` hacia el servidor central. |
...
Privilege), el patrón más repetido es **el mismo gap apareciendo bajo
cuatro categorías STRIDE distintas**: `POST
/events`, `GET /health`, y `GET /stats` en `rpi/proxy.py` no tienen
ningún mecanismo de autenticación en su definición — no por una
variable de entorno mal configurada, sino porque la función
`receive_events()` (y las dos de solo lectura) simplemente no declaran
ningún parámetro, header, ni dependency de FastAPI que verifique al
emisor (S-3, T-3 parcialmente relacionado, R-3, D-3, I-3 — cinco
amenazas, un solo origen estructural). Esto no es casualidad — un punto
de ingesta sin autenticación obligatoria abre simultáneamente spoofing,
tampering, repudiation, DoS, y disclosure, porque todas dependen de la
misma garantía ausente: saber quién está hablando con el proxy.

**Nota importante de esta revisión:** `VTR_API_KEY` existe en el código
pero protege un canal distinto — la autenticación del *proxy hacia el
servidor central* al sincronizar (`SyncConfig(api_key=...)`), no la
ingesta de eventos entrantes. Configurar esa variable no cierra ninguna
de las cinco amenazas de esta sección; son gaps independientes que
requieren su propio mecanismo (ej. un header de API key específico para
`receive_events()`, o autenticación a nivel de red/firewall en el
despliegue).

**Recomendación de prioridad, no instrucción — la decisión de
implementación es de Luis:**

1. **Más alto impacto, menor esfuerzo:** agregar un mecanismo de
   autenticación real a `POST /events`, `GET /health`, y `GET /stats`
   (ej. header de API key verificado vía dependency de FastAPI) — esto
   es trabajo nuevo, no simplemente "configurar una variable que ya
   existe", porque `VTR_API_KEY` actual no sirve para este propósito.
2. **Rate limiting en `POST /events`** (D-3) — no existe hoy, y combinado
   con el punto 1 reduce significativamente la superficie de DoS desde
   la red local de planta.
3. **Sincronización de revocación de tokens tras reinicio** (R-1) — ya
   está en el roadmap como dependencia de `CustodyManager`, que ya existe
   como módulo (`core/custody_manager.py`) pero su integración específica
   con la lista de revocación de `jwt_verifier.py` no se confirmó en esta
   revisión.
4. **Verificación de firma en config de campo** (S-5, Q-03) — ya tiene
   decisión documentada, solo falta implementación, ver checklist en
   `docs/DOD-v0.5.0.md` §5.

Ninguno de estos cuatro puntos está dentro del alcance de las 10
propuestas de la fase criptográfica ni del checklist de 14 puntos
post-#10 tal como estaba escrito antes de este documento — STRIDE
encontró superficie nueva (específicamente en `rpi/proxy.py`) que ningún
documento anterior había catalogado con este nivel de detalle.
